...
try
    ...
    %% create CFS rectangles and textures
    CFSTextures = cell(Settings.CFS.N,1);
    masks = make_mondrian_masks(Settings.StimSize,Settings.StimSize,Settings.CFS.N,Settings.CFS.Shape,Settings.CFS.Selection,Settings.CFS.SizeRect);
    for iCFS = 1:Settings.CFS.N
      %  masks{iCFS}=masks{iCFS}(:,:,1);      
  %     imwrite(masks{iCFS},fullfile(Settings.rootdir,[num2str(iCFS) '.bmp']),'bmp');       
        masks{iCFS}=round(masks{iCFS}*255);                      
        CFSTextures{iCFS}=Screen('MakeTexture', w, masks{iCFS}, 0, 0, 0, 0, 0);
    end
...
% select 8 masks for this trial

mymasks = randperm(Settings.CFS.N);
mymasks = mymasks(1:8);

Results.mymasks{mytrialnum} = mymasks;
% frame based rsvp descriptors
DomEye_Textures = [ones(1,6*7)*GreyTexture]; % 7 * 6 frames à 16.67ms
NonDomEye_Textures = [ones(1,6)*CFSTextures{mymasks(1)} ones(1,6)*CFSTextures{mymasks(2)} ones(1,6)*CFSTextures{mymasks(3)} ones(1,6)*CFSTextures{mymasks(4)}...
    ones(1,6)*CFSTextures{mymasks(5)} ones(1,6)*CFSTextures{mymasks(6)} ones(1,6)*CFSTextures{mymasks(7)}];
    ...
    frame_counter = 0;    
        while 1
            ...
            Screen('DrawTexture', w, FrameTexture,[], recter(Settings.FrameSize, [xpos(Settings.NonDomEye) ypos], [x y]), [], [], 1);
            Screen('DrawTexture', w, FrameTexture,[], recter(Settings.FrameSize, [xpos(Settings.DomEye) ypos], [x y]), [], [], 1);

            Screen('DrawTexture', w, DomEye_Textures(frame_counter) ,[], recter(Settings.StimSize, [xpos(Settings.DomEye) ypos], [x y]), [], [], 1);
            Screen('DrawTexture', w, NonDomEye_Textures(frame_counter) ,[], recter(Settings.StimSize, [xpos(Settings.NonDomEye) ypos], [x y]), [], [], 1);
            
%             if Settings.NonDomEye == 1
%                 makefix_onlyright(xpos,ypos,Settings.Fixation.Size,Settings.Fixation.Colour,w);
%             else
%                 makefix_onlyleft(xpos,ypos,Settings.Fixation.Size,Settings.Fixation.Colour,w);
%             end;
    ...
    %        WaitSecs(0.01);
       ...
       %     if (GetSecs-myzero)>myprimeduration/1000 break; end;

            if frame_counter == length(DomEye_Textures) break; end;
...
Screen('DrawTexture', w, GreyTexture,[], recter(Settings.StimSize, [xpos(1) ypos], [x y]), [], [], 1);
Screen('DrawTexture', w, GreyTexture,[], recter(Settings.StimSize, [xpos(2) ypos], [x y]), [], [], 1);
[nx, ny, bbox] = DrawFormattedText(w, 'NO RESPONSE!',  xpos(1)-Settings.FrameSize/2+Settings.FrameWidth + x_offset, ypos-Settings.FrameSize/4, Settings.Red);
[nx, ny, bbox] = DrawFormattedText(w, 'NO RESPONSE!',  xpos(2)-Settings.FrameSize/2+Settings.FrameWidth + x_offset, ypos-Settings.FrameSize/4, Settings.Red);
Screen('Flip',w);
...
Screen('DrawTexture', w, GreyTexture,[], recter(Settings.StimSize, [xpos(1) ypos], [x y]), [], [], 1);
Screen('DrawTexture', w, GreyTexture,[], recter(Settings.StimSize, [xpos(2) ypos], [x y]), [], [], 1);
[nx, ny, bbox] = DrawFormattedText(w, 'ENDE',xpos(1)-Settings.FrameSize/2+Settings.FrameWidth+ x_offset, ypos-Settings.FrameSize/4 , Settings.Writing.Colour);
[nx, ny, bbox] = DrawFormattedText(w, 'ENDE',  xpos(2)-Settings.FrameSize/2+Settings.FrameWidth+ x_offset, ypos-Settings.FrameSize/4  , Settings.Writing.Colour);
Screen('Flip',w);
